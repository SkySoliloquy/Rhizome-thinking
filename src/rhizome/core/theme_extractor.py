...
class ThemeExtractor:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("MiniMax API key is required for theme extraction.")
    ...
    async def extract_themes_from_node(self, node: Node) -> list[dict[str, Any]]:
        ...
        try:
            ...
            print(f"Failed to extract themes from node {node.id}: {e}")
...
class MockThemeExtractor(ThemeExtractor):
    """Mock theme extractor for testing without API calls."""
    
    def __init__(self, *args, **kwargs) -> None:
        """Initialize mock extractor without API key check."""
        self.api_key = "mock"
        self.base_url = "mock"
        self.model = "mock"
